angular.module('page', ["ideUI", "ideView", "entityApi"])
	.config(["messageHubProvider", function (messageHubProvider) {
		messageHubProvider.eventIdPrefix = 'codbex-petstore.entities.orderStatus';
	}])
	.config(["entityApiProvider", function (entityApiProvider) {
		entityApiProvider.baseUrl = "/services/js/codbex-petstore/gen/api/entities/orderStatus.js";
	}])
		...
		$scope.loadPage = function (pageNumber) {
			...
			entityApi.count().then(function (response) {
				...
				entityApi.list(offset, limit).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("orderStatus", `Unable to list orderStatus: '${response.message}'`);
						return;
					}
